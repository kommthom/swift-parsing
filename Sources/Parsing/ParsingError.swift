...
enum ParsingError: Error {
	...
	@usableFromInline
	static func expectedInput(_ description: String, at remainingInput: Sendable) -> Self {
		.failed(
	...
	static func expectedInput(
		...
		from originalInput: Sendable,
		to remainingInput: Sendable
	) -> Self {
		.failed(
			summary: "unexpected input",
			label: "expected \(description)",
	...
	@usableFromInline
	static func failed(summary: String, label: String = "", at remainingInput: Sendable) -> Self {
		.failed(label, .init(remainingInput: remainingInput, debugDescription: summary))
	}
	
	@usableFromInline
	static func failed(
		summary: String, label: String = "", from originalInput: Sendable, to remainingInput: Sendable
	) -> Self {
		.failed(
			label,
			.init(
	...
	@usableFromInline
	static func manyFailed(_ errors: [Error], at remainingInput: Sendable) -> Self {
		.manyFailed(errors, .init(remainingInput: remainingInput, debugDescription: ""))
	}
	
	@usableFromInline
	static func wrap(_ error: Error, from originalInput: Sendable, to remainingInput: Sendable) -> Self {
	...
	@usableFromInline
	static func wrap(_ error: Error, at remainingInput: Sendable) -> Self {
	...
	struct Context: Sendable {
		...
		@usableFromInline
		var originalInput: Sendable
		
		@usableFromInline
		var remainingInput: Sendable
		...
		@usableFromInline
		init(
			originalInput: Sendable,
			remainingInput: Sendable,
...
extension ParsingError.Context {
	fileprivate func canGroup(with other: Self) -> Bool {
		...
		return (areSame(self.remainingInput, other.remainingInput)
...
func format(labels: [String], context: ParsingError.Context) -> String {
   ...
   """
				...
				return formatError(
					...
					location: "input:\(position.line + 1):\(position.column + 1)\(through.line == position.line ? (through.column <= position.column + 1 ? "" : "-\(through.column)") : "-\(through.line + 1):\(through.column + 1)")",
