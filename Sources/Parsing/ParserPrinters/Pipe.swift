...
extension ParserProtocol {
	...
	public func pipe<Downstream: Sendable>(@ParserBuilder<Output> _ build: @Sendable () -> Downstream) -> Parsers.Pipe<Self, Downstream> {
		.init(upstream: self, downstream: build())
	}
}

extension ParserProtocol where Input: Collection {
	public func pipe<Downstream>(@ParserBuilder<Output> _ build: @Sendable () -> Downstream) -> Parsers.Pipe<Self, ParserBuilder<Input>.SkipSecond<Downstream, Parsers.PipeEnd<Self.Input>>> {
		.init(
			upstream: self,
...
extension Parsers {
	...
	public struct Pipe<Upstream: ParserProtocol, Downstream: ParserProtocol>: ParserProtocol where Upstream.Output == Downstream.Input {
	...
	public struct PipeEnd<Input: Collection & Sendable>: ParserProtocol where Input.SubSequence: Sendable {
		@usableFromInline
		init() {}
...
extension Parsers.Pipe: ParserPrinterProtocol & SendableMarker where Upstream: ParserPrinterProtocol, Downstream: ParserPrinterProtocol, Upstream.Output: _EmptyInitializable {
	@inlinable
...
extension Parsers.PipeEnd: ParserPrinterProtocol {
	@inlinable
