...
/// ```
public struct Digits<Length: CountingRangeProtocol & Sendable, InputToBytes: ConversionProtocol>: ParserProtocol where InputToBytes.Input: Collection, InputToBytes.Input.SubSequence == InputToBytes.Input, InputToBytes.Output: Collection, InputToBytes.Output.Element == UTF8.CodeUnit, InputToBytes.Output.SubSequence == InputToBytes.Output {
	@usableFromInline
	let length: Length
	...
	@inlinable
	public func parse(_ input: inout InputToBytes.Input) throws -> Int where InputToBytes.Input: Sendable {
  ...
  to: input
		...
		while self.length.maximum.map({ length < $0 }) ?? true, let next = iterator.next(), let n = digit(for: next) {
			bytes.removeFirst()
		...
		guard length >= self.length.minimum else { throw digitsError() }
		input = try self.inputToBytes.unapply(bytes)
		return output
	}
}

extension Digits: ParserPrinterProtocol where InputToBytes.Input: PrependableCollectionProtocol, InputToBytes.Output: PrependableCollectionProtocol{
	@inlinable
	public func print(_ output: Int, into input: inout InputToBytes.Input) throws {
		guard self.length.minimum != 0 || output != 0 else { return }
		guard output >= 0 else {
...
extension Digits {
	@inlinable
	public init<C>() where Length == PartialRangeFrom<Int>, InputToBytes == Conversions.IdentityIso<C> {
	self.init(1...)
	}
	
	@inlinable
	public init<C>(_ length: Length) where InputToBytes == Conversions.IdentityIso<C> {
		self.init(length: length, inputToBytes: .init())
	}
}

extension Digits where InputToBytes == Conversions.SubstringToUTF8ViewIso {
	@_disfavoredOverload
	@inlinable
...
extension Digits where InputToBytes == Conversions.IdentityIso<Substring.UTF8View> {
	@_disfavoredOverload
	@inlinable
	public init() where Length == PartialRangeFrom<Int> {
		self.init(1...)
