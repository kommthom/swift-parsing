...
extension Bool {
	...
	///   code units.
	@inlinable
	public static func parser<Input: Sendable>(of inputType: Input.Type = Input.self) -> Parsers.BoolParser<Input> {
		.init()
	}
}
...
extension Parsers {
	...
	///
	/// See <doc:Bool> for more information about this parser.
	public struct BoolParser<Input: Collection & Sendable>: ParserProtocol where Input.SubSequence == Input, Input.Element == UTF8.CodeUnit {
		@inlinable
		public init() {}
...
extension Parsers.BoolParser: ParserPrinterProtocol where Input: PrependableCollectionProtocol {
	@inlinable
