...
public struct Rest<Input: Collection & Sendable>: ParserProtocol where Input.SubSequence == Input {
	@inlinable
	public init() {}
	
	@inlinable
	public func parse(_ input: inout Input) throws -> Input {
		guard !input.isEmpty else { throw ParsingError.expectedInput("a non-empty input", at: input) }
		let output = input
...
extension Rest: ParserPrinterProtocol where Input: PrependableCollectionProtocol {
	@inlinable
