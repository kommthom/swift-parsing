...
extension ParserProtocol {
	...
	@inlinable
	public func printing<P: ParserProtocol>(_ printer: P) -> Parsers.OverridePrinting<Self, P> {
		.init(
			parser: self,
			printer: printer
		)
	...
	@inlinable
	public func printing(_ print: @escaping @Sendable (Output, inout Input) -> Void) -> Parsers.Print<Self> {
		.init(
			parser: self,
			printer: print
		)
	...
	@inlinable
	public func printing(_ print: @escaping PrintingAlias<Input, Output>) -> Parsers.TryPrint<Self> {
		.init(
			parser: self,
			printer: print
		)
...
extension Parsers {
	...
	public struct OverridePrinting<Parser: ParserProtocol, Printer: ParserPrinterProtocol>: ParserPrinterProtocol where Parser.Input == Printer.Input, Parser.Output == Printer.Output {
		public let parser: Parser
		public let printer: Printer
		
		@inlinable
		public init(parser: Parser,	printer: Printer) {
			self.parser = parser
	...
	public struct Print<Upstream: ParserProtocol>: ParserPrinterProtocol {
		public let parser: Upstream
		public let printer: @Sendable (Upstream.Output, inout Upstream.Input) -> Void
		
		@inlinable
		public init(parser: Upstream, printer: @escaping @Sendable (Upstream.Output, inout Upstream.Input) -> Void) {
			self.parser = parser
			self.printer = printer
		}
	...
	public struct TryPrint<Upstream: ParserProtocol>: ParserPrinterProtocol {
		public let parser: Upstream
		public let printer: @Sendable (Upstream.Output, inout Upstream.Input) throws -> Void
		
		@inlinable
		public init(parser: Upstream, printer: @escaping @Sendable (Upstream.Output, inout Upstream.Input) throws -> Void) {
			self.parser = parser
			self.printer = printer
		}
