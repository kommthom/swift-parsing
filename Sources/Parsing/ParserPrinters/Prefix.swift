...
/// ```
public struct Prefix<Input: Collection & Sendable>: ParserProtocol where Input.SubSequence == Input {
	public let maximum: Int?
	public let minimum: Int
	public let predicate: (@Sendable (Input.Element) -> Bool)?
	...
	@inlinable
	public init<R: CountingRangeProtocol>(_ length: R, while predicate: (@Sendable (Input.Element) -> Bool)? = nil) {
	...
	@inlinable
	public init(while predicate: @escaping @Sendable (Input.Element) -> Bool) {
		self.minimum = 0
		self.maximum = nil
		self.predicate = predicate
	}
...
extension Prefix: ParserPrinterProtocol & SendableMarker where Input: PrependableCollectionProtocol {
	@inlinable
