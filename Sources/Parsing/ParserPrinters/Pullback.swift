...
extension ParserProtocol {
	...
	@inlinable
	public func pullback<NewInput: Sendable>(_ keyPath: Sendable & WritableKeyPath<NewInput, Input>) -> Parsers.Pullback<Self, NewInput> {
...
extension Parsers {
	...
	public struct Pullback<Downstream: ParserProtocol, Input: Sendable>: ParserProtocol {
		public let downstream: Downstream
		public let keyPath: Sendable & WritableKeyPath<Input, Downstream.Input>
		
		@inlinable
		public init(downstream: Downstream, keyPath: Sendable & WritableKeyPath<Input, Downstream.Input>) {
		...
		@inlinable
		public func pullback<NewInput>(_ keyPath: Sendable & WritableKeyPath<NewInput, Input>) -> Pullback<Downstream, NewInput> {
			.init(downstream: self.downstream, keyPath: keyPath.appending(path: self.keyPath) as! Sendable & WritableKeyPath<NewInput, Downstream.Input>)
		}
	}
}

extension Parsers.Pullback: ParserPrinterProtocol & SendableMarker where Downstream: ParserPrinterProtocol {
