		...
		func consumeHorizontal() -> Bool {
			...
			switch bytes.first {
				...
				case 194:  // U+00A0
					...
					return false
				case 225:  // U+1680
					if bytes.dropFirst().first == 154, bytes.dropFirst(2).first == 128 {
						bytes.removeFirst(3)
						return true
					}
					return false
				case 226:  // U+2000 ~ U+200A, U+202F
					switch bytes.dropFirst().first {
						case 128:
							if let byte = bytes.dropFirst(2).first, (128...138).contains(byte) || byte == 175 {
								bytes.removeFirst(3)
								return true
							}
							return false
		...
		func consumeVertical() -> Bool {
			...
			switch bytes.first {
				...
				case 226:  // U+2028, U+2029
					if bytes.dropFirst().first == 128, let byte = bytes.dropFirst(2).first, byte == 168 || byte == 169 {
						bytes.removeFirst(3)
						return true
					}
					return false
...
extension Whitespace: ParserPrinterProtocol & SendableMarker
where InputToBytes.Input: PrependableCollectionProtocol, InputToBytes.Output: PrependableCollectionProtocol {
	@inlinable
...
extension Whitespace {
	@inlinable
	public init<C: Sendable>(_ configuration: Configuration = .all) where Length == PartialRangeFrom<Int>, InputToBytes == Conversions.IdentityIso<C> {
		self.init(0..., configuration)
	}
	
	@inlinable
	public init<C>(_ length: Length, _ configuration: Configuration = .all) where InputToBytes == Conversions.IdentityIso<C> {
		self.length = length
